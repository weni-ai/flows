...
class InstagramTypeTest(TembaTest):
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_refresh_token_without_token(self, mock_get, mock_post):
        token = "x" * 200
        ...
        mock_get.side_effect = [
            MockResponse(400, json.dumps({"error": "token invalid"})),
            MockResponse(200, json.dumps({"data": {"is_valid": False}})),
            MockResponse(200, json.dumps({"data": {"is_valid": True}})),
            MockResponse(200, json.dumps({"access_token": self.long_life_page_token})),
            MockResponse(
                200,
                json.dumps(
                    {"paging": {"next": "new_url"}, "data": [{"id": "1", "access_token": "", "name": "pagina1"}]}
                ),
            ),
            MockResponse(
    ...
    def test_claim(self, mock_get, mock_post):
        ...
        channel = Channel.objects.get(address="1234567", channel_type="IG")
        self.assertEqual(channel.config[Channel.CONFIG_AUTH_TOKEN], self.long_life_page_token)
        self.assertEqual(channel.config[Channel.CONFIG_PAGE_NAME], "Temba")
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_execute_action_error_page_id(self, mock_get, mock_post):
        token = "x" * 200
        ...
        self.channel.config["page_id"] = None
        self.channel.save()
