...
from temba.orgs.models import PLIVO_AUTH_ID, PLIVO_AUTH_TOKEN, PLIVO_APP_ID
...
class OrgTest(TembaTest):
    ...
    def test_connect_plivo(self):
        ...
        with patch('requests.get') as plivo_mock:
            ...
            self.assertFalse(self.org.is_connected_to_plivo())

        # ok, now with a success
        with patch('requests.get') as plivo_mock:
            plivo_mock.return_value = MockResponse(200, json.dumps(dict(objects=[dict(app_name='%s/%d' % (settings.TEMBA_HOST.lower(), self.org.pk),
                                                                                      app_id='application-id')])))
            self.client.post(connect_url, dict(auth_id='auth-id', auth_token='auth-token'))

            # plivo should now be connected
            self.org = Org.objects.get(pk=self.org.pk)
            self.assertTrue(self.org.is_connected_to_plivo())
            self.assertEquals(self.org.config_json()[PLIVO_AUTH_ID], 'auth-id')
            self.assertEquals(self.org.config_json()[PLIVO_AUTH_TOKEN], 'auth-token')
            self.assertEquals(self.org.config_json()[PLIVO_APP_ID], 'application-id')
