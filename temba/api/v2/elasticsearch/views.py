...
class ContactsElasticSearchEndpoint(APIView):
    ...
    def get(self, request, *args, **kwargs):  # pragma: no cover
        ...
        if name or number:
            ...
            contacts = Search(using=client, index=index).query(qs).params(size=page_size, from_=from_index)
            response = list(contacts.scan())
            
            results = [hit.to_dict() for hit in response]

            total_results = response.hits.total.value if hasattr(response.hits, 'total') and hasattr(response.hits.total, 'value') else len(results)
